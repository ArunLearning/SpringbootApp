...
a. Run a container in default network :
```
docker create -itd --restart unless-stopped -p 8180:8080 --name sbdc springbootdocker -e DBIP=172.17.0.1 -e DBPORT=3306 -e DBUSR=root -e DBPWD=password123
```
b. Run a container in differrent network (create docker network before assigning) :
```
docker run -itd --restart unless-stopped -p 8888:8080 --network bridge2 --name sbd -e DBIP=172.18.0.1 -e DBPORT=3307 -e DBUSR=root -e DBPWD=password123 spdimg
```
```
docker ps
```
